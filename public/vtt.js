(() => {
  ...
  function inspectorMarkup(entries, isDm, clientId) {
    ...
    if(entry.kind==="token"){
      ...
      return `<div class="vtt-inspector-card"><div class="vtt-inspector-title"><span class="vtt-color-dot" style="background:${esc(value.color||"#9f7842")}"></span><div><small>${value.playerId?"Персонаж":"Токен"}</small><strong>${esc(value.name)}</strong></div></div><div class="vtt-stat-grid"><span><small>X / Y</small><b>${Number(value.x)} / ${Number(value.y)}</b></span><span><small>Размер</small><b>${Number(value.size)}</b></span><span><small>HP</small><b>${Number(value.hp||0)}/${Number(value.hpMax||0)}</b></span><span><small>Инициатива</small><b>${value.initiative??"—"}</b></span></div><div class="vtt-inspector-actions">${canEdit?`<button data-vtt-edit-token="${esc(value.id)}">Настроить</button>`:""}${!value.playerId&&(isDm||npcSheetHasData(value.npcSheet))?`<button data-vtt-open-npc="${esc(value.id)}">Лист NPC</button>`:""}<button data-vtt-roll="${esc(value.id)}" ${canEdit?"":"disabled"}>Инициатива</button>${isDm?`<button data-vtt-duplicate-selected>Копировать</button><button class="danger-action" data-vtt-remove-selected>Удалить</button>`:""}</div></div>`;
    }
  ...
  function characterPanelMarkup(characters, selectedId, isDm, clientId, scene) {
    ...
    const spellMarkup=spells.map(spell=>`<article class="vtt-character-spell-row ${spell.summon?"is-summon":""}"><div><i>${(prefs.pinnedSpells||[]).includes(spell.id)?"★":spell.summon?"♙":spell.concentration?"◉":"✧"}</i><span><strong>${esc(spell.name)}</strong><small>${spell.level?`${spell.level} ур.`:"заговор"}${spell.ritual?" · ритуал":""}${spell.sourceId==="xgte"?" · XGtE":spell.sourceId==="tcoe"?" · TCoE":""}${spell.summon?" · призыв":""}</small></span></div>${spell.formula?rollButton(spell.formula,`${spell.kind==="healing"?"Лечение":"Заклинание"}: ${spell.name}`,esc(spell.formula),"vtt-formula-button"):""}<button type="button" data-vtt-cast-spell="${esc(spell.id)}" ${canRoll?"":"disabled"}>Сотворить</button>${isDm&&spell.summon?`<button type="button" class="vtt-summon-marker" data-vtt-place-summon="${esc(spell.id)}" data-player-id="${esc(character.playerId)}" title="Поставить редактируемый маркер рядом с токеном заклинателя">＋ Маркер</button>`:""}</article>`).join("");
