(() => {
  ...
  const ui = {
    ...
    npcPage: ["overview","attacks","saves","skills-a","skills-b","features","spells"].includes(savedUi.npcPage) ? savedUi.npcPage : "overview",
  ...
  const saveUiState = () => { try { localStorage.setItem(UI_STORAGE_KEY,JSON.stringify({ rightPanel:ui.rightPanel, characterPage:ui.characterPage, characterChecksPage:ui.characterChecksPage, npcPage:ui.npcPage, fogShape:ui.fogShape, movementSnap:ui.movementSnap !== false })); } catch {} };
  ...
  function npcSheetHasData(sheet) {
    ...
    return Object.keys(sheet.stats || {}).length > 0 || ["saves","checks","attacks","formulas","features","spells"].some(key => Array.isArray(sheet[key]) && sheet[key].length);
  }
  ...
  function npcCharacterPanelMarkup(token, isDm) {
    ...
    const roll=(formula,label,content,entry={public:true},className="")=>formula?`<button type="button" class="${className}" data-vtt-npc-formula="${esc(formula)}" data-vtt-npc-label="${esc(label)}" data-vtt-npc-visibility="${entry.public?"public":"private"}" ${isDm?"":"disabled"}>${content}</button>`:`<div class="${className} is-readonly">${content}</div>`;
    ...
    const checkRow=(entry,prefix)=>roll(entry.formula,`${prefix}: ${token.name} · ${entry.name}`,`<i>${entry.public?"◆":"◇"}</i><em>${esc(entry.name)}</em><b>${esc(entry.formula)}</b>${badge(entry)}`,entry);
    const saves=(sheet.saves||[]).map(entry=>checkRow(entry,"Спасбросок NPC")).join("");
    const checks=sheet.checks||[];
    const skillMiddle=Math.ceil(checks.length/2);
    const attackRows=(sheet.attacks||[]).map(entry=>`<article><div><strong>${esc(entry.name)}</strong><small>${esc([entry.category,entry.damageType||"Атака"].filter(Boolean).join(" · "))}${isDm?` · ${entry.public?"публично":"скрыто"}`:""}</small>${entry.text?`<p>${esc(entry.text)}</p>`:""}</div><div class="vtt-npc-attack-actions">${entry.attackFormula?roll(entry.attackFormula,`Атака NPC: ${token.name} · ${entry.name}`,esc(entry.attackFormula),entry,"vtt-formula-button"):""}${entry.damageFormula?roll(entry.damageFormula,`Урон NPC: ${token.name} · ${entry.name}`,esc(entry.damageFormula),entry,"vtt-formula-button damage"):""}</div></article>`).join("");
    const featureNames=new Set((sheet.features||[]).map(entry=>String(entry.name||"").trim().toLocaleLowerCase("ru")));
    const featureItems=[...(sheet.features||[]),...(sheet.formulas||[]).filter(entry=>!featureNames.has(String(entry.name||"").trim().toLocaleLowerCase("ru"))).map(entry=>({...entry,category:"Своя формула"}))];
    const featureRows=featureItems.map(entry=>`<article class="vtt-npc-feature-card"><div><small>${esc(entry.category||"Особенность")}${isDm?` · ${entry.public?"публично":"скрыто"}`:""}</small><strong>${esc(entry.name)}</strong>${entry.text?`<p>${esc(entry.text)}</p>`:""}</div>${entry.formula?roll(entry.formula,`${entry.category||"Особенность"}: ${token.name} · ${entry.name}`,esc(entry.formula),entry,"vtt-formula-button"):""}</article>`).join("");
    const spellRows=(sheet.spells||[]).map(entry=>`<article class="vtt-character-spell-row"><div><i>✧</i><span><strong>${esc(entry.name)}</strong><small>${entry.level?`${Number(entry.level)} ур.`:esc(entry.category||"магия")}${isDm?` · ${entry.public?"публично":"скрыто"}`:""}</small>${entry.text?`<p>${esc(entry.text)}</p>`:""}</span></div>${entry.formula?roll(entry.formula,`Заклинание NPC: ${token.name} · ${entry.name}`,esc(entry.formula),entry,"vtt-formula-button"):""}</article>`).join("");
    const sections=["overview","attacks","saves","skills-a","skills-b","features",...((sheet.spells||[]).length?["spells"]:[])];
    const page=sections.includes(ui.npcPage)?ui.npcPage:"overview";
    const labels={overview:"Обзор",attacks:"Атаки",saves:"Спасы","skills-a":"Навыки 1","skills-b":"Навыки 2",features:"Умения",spells:"Магия"};
    const tabs=sections.map(section=>`<button type="button" data-vtt-npc-page="${section}" class="${page===section?"active":""}">${labels[section]}</button>`).join("");
    const dispositionLabel={friendly:"Союзник",neutral:"Нейтральный",hostile:"Противник"}[token.disposition]||"NPC";
    const overview=`<div class="vtt-character-vitals vtt-npc-vitals"><article><small>HP</small><strong>${Number(token.hp||0)}/${Number(token.hpMax||1)}${Number(token.tempHp)?` +${Number(token.tempHp)}`:""}</strong></article><article><small>КД</small><strong>${Number(token.ac||0)}</strong></article><article><small>Инициатива</small><strong>${bonusText(token.initiativeBonus)}</strong></article><article><small>Зрение</small><strong>${Number(token.vision||0)} фт.</strong></article><article><small>Опасность</small><strong>${profile.cr?`CR ${esc(profile.cr)}`:"—"}</strong></article><article><small>Пассивка</small><strong>${Number(profile.passivePerception||0)||"—"}</strong></article></div><section class="vtt-character-section"><h4>Характеристики</h4><div class="vtt-character-abilities">${statMarkup||`<div class="vtt-empty-side">Характеристики скрыты.</div>`}</div></section>${profile.type||profile.speed||profile.senses||profile.languages||profile.defenses?`<section class="vtt-character-section"><h4>Сведения</h4><div class="vtt-npc-profile">${profile.type?`<article><small>Тип</small><strong>${esc(profile.type)}</strong></article>`:""}${profile.size?`<article><small>Размер</small><strong>${esc(profile.size)}</strong></article>`:""}${profile.speed?`<article><small>Скорость</small><strong>${esc(profile.speed)}</strong></article>`:""}${profile.senses?`<article><small>Чувства</small><strong>${esc(profile.senses)}</strong></article>`:""}${profile.languages?`<article><small>Языки</small><strong>${esc(profile.languages)}</strong></article>`:""}${profile.defenses?`<article class="wide"><small>Защиты</small><strong>${esc(profile.defenses)}</strong></article>`:""}</div></section>`:""}`;
    const attacks=`<section class="vtt-character-section"><h4>Атаки и урон</h4><div class="vtt-character-attacks">${attackRows||`<div class="vtt-empty-side">Атак нет.</div>`}</div></section>`;
    const savePage=`<section class="vtt-character-section"><h4>Спасброски</h4><div class="vtt-character-checks">${saves||`<div class="vtt-empty-side">Спасброски не заданы.</div>`}</div></section>`;
    const skillsPage=(items,title)=>`<section class="vtt-character-section"><h4>${title}</h4><div class="vtt-character-checks skills">${items.map(entry=>checkRow(entry,"Проверка NPC")).join("")||`<div class="vtt-empty-side">Навыки не заданы.</div>`}</div></section>`;
    const features=`<section class="vtt-character-section"><h4>Особенности и действия</h4><div class="vtt-npc-feature-list">${featureRows||`<div class="vtt-empty-side">Особенности не заданы.</div>`}</div></section>`;
    const spells=`<section class="vtt-character-section"><h4>Заклинания и магия</h4><div class="vtt-character-spells">${spellRows||`<div class="vtt-empty-side">Заклинаний нет.</div>`}</div></section>`;
    const content={overview,attacks,saves:savePage,"skills-a":skillsPage(checks.slice(0,skillMiddle),"Навыки 1"),"skills-b":skillsPage(checks.slice(skillMiddle),"Навыки 2"),features,spells}[page]||overview;
  ...
  function bind(root, ctx, signal, metrics) {
    ...
    root.querySelectorAll("[data-vtt-npc-page]").forEach(button => button.addEventListener("click", () => {
      ui.npcPage = button.dataset.vttNpcPage;
      saveUiState();
      render(root,ctx);
    }, { signal }));
