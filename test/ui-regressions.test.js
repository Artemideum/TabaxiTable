...
test("версии схем листа и сцены не размазаны магическими числами", () => {
  ...
  assert.match(server, /const SCENE_SCHEMA_VERSION = 13;/);
...
test("быстрый лист NPC разбит на вкладки и прокручивается колесом", () => {
  assert.match(vtt, /npcPage: \["overview","attacks","saves","skills-a","skills-b","features","spells"\]/);
  assert.match(vtt, /data-vtt-npc-page/);
  assert.match(vtt, /ui\.npcPage = button\.dataset\.vttNpcPage/);
  assert.match(vtt, /Навыки 1/);
  assert.match(vtt, /Навыки 2/);
  assert.match(vtt, /Особенности и действия/);
  assert.match(vttCss, /\.vtt-npc-tabs[\s\S]*overflow-x:auto/);
  assert.match(vttCss, /\.vtt-npc-sheet \.vtt-character-page[\s\S]*overflow-y:scroll/);
  assert.match(vtt, /querySelectorAll\("\.vtt-character-page"\)[\s\S]*wheel[\s\S]*stopPropagation/);
});

test("массовый каталог показывает существ порциями и поддерживает сложные разделы",()=>{
  assert.match(bestiaryClient,/const PAGE_SIZE=60/);
  assert.match(bestiaryClient,/data-bestiary-more/);
  assert.match(bestiaryClient,/slice\(0,state\.visibleCount\)/);
  assert.match(bestiaryClient,/Мифические действия/);
  assert.match(bestiaryClient,/Действия логова/);
  assert.match(bestiaryClient,/Региональные эффекты/);
  assert.match(bestiaryClient,/Заклинания/);
});

test("расы Monster Manual подключены как отдельный контент-пак до правил",()=>{
  const generated=fs.readFileSync("public/races-mm-generated.js","utf8");
  assert.ok(indexHtml.indexOf('/races-mm-generated.js')>-1);
  assert.ok(indexHtml.indexOf('/races-mm-generated.js')<indexHtml.indexOf('/rules-5e.js'));
  assert.match(generated,/mm14-races/);
  assert.match(generated,/"mm-goblin"/);
  assert.match(generated,/"mm-yuan-ti-pureblood"/);
  assert.match(generated,/"flexible": \[\s*2,\s*1/);
});


test("локальные портреты бестиария всегда перепроверяются после ремонта", () => {
  assert.match(server, /app\.use\("\/bestiary-content"[\s\S]*maxAge:0[\s\S]*Cache-Control","no-cache, max-age=0, must-revalidate"/);
});
